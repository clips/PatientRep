...
#         pred_score = fit_model.predict_proba_generator(generator = nn_utils.batch_generator(
#                                                                      x_test, 
#                                                                      None,
#                                                                      batch_size = self.batch_size, 
#                                                                      shuffle = False, 
...
#         print(recon_mse)
    ...
    def get_pretrained_sda_multistage(self, data_train, data_val, data_test, vocab_freq, vocab_indices, dir_out):
        
        data_train_subset = data_train.tolil()
        data_val_subset = data_val.tolil()
        data_test_subset = data_test.tolil()
        
        #sort term in ascending order: low freq terms first
        vocab_freq = OrderedDict(sorted(vocab_freq.items(), key=operator.itemgetter(1)))
        
        #reset least frequent feature values to 0
        for i, term in enumerate(vocab_freq.keys()):
            if term == 'OOV':
                continue
            data_train_subset[:,vocab_indices[term]] = 0
            data_val_subset[:,vocab_indices[term]] = 0
            data_test_subset[:,vocab_indices[term]] = 0
            
            if i == np.ceil(0.90*len(vocab_freq)):
                print("Breaking after ", i, " features")
                break
        
        
        #pass feat subset as parameters to get_pretrained_sda, write_model = False, get_sup_model = False. Get list of trained layers.
        model_layers, (_, _, _), recon_mse_stage1 = self.get_pretrained_sda(data_train_subset.tocsr(), data_val_subset.tocsr(), data_test_subset.tocsr(), dir_out, get_enc_model = False, write_model = False)
        
        #continue_sdae_training with that list as parameter, get_sup_model, write_model as True
        model, (dense_train, dense_val, dense_test), recon_mse_stage2 = self.get_pretrained_sda(data_train, data_val, data_test, dir_out, get_enc_model = True, write_model = True, model_layers = model_layers)
        
        return model, (dense_train, dense_val, dense_test), recon_mse_stage1, recon_mse_stage2
